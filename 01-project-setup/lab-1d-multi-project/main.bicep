...
var aiAccountName = 'foundry-multi-${suffix}'
...
resource projects 'Microsoft.CognitiveServices/accounts/projects@2025-04-01-preview' = [
  for i in range(0, projectCount): {
    ...
    identity: { type: 'SystemAssigned' }
    properties: {
      description: 'Project for team ${teamNames[i]} connecting to central landing zone via APIM'
...
resource apimConnections 'Microsoft.CognitiveServices/accounts/projects/connections@2025-04-01-preview' = [
  for i in range(0, projectCount): {
    parent: projects[i]
    name: 'landing-zone-apim-${teamNames[i]}'
    properties: {
...
output apimConnectionNames array = [for i in range(0, projectCount): 'landing-zone-apim-${teamNames[i]}']
